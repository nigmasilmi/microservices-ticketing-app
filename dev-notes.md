...
make changes for a service => commit to a git branch !== master => push to github => cfreate a manual pull request to merch branch into master => Github runs all tests => Merge pull request after the tests passed
...
4. Configuration of name, on (when to trigger)
